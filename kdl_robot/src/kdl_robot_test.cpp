...
int main(int argc, char **argv)
{
    ...
    std::vector<KDL::Frame> targets;

    // starting point
    KDL::Frame start_frame = robot.getEEFrame();
    KDL::Rotation fixed_orientation = start_frame.M;
    KDLPlanner planner;
    ...
    // initialize planner with first target
    planner.setWaypointsFrames({start_frame, targets[0]}, {traj_duration});
    ...
    while (ros::ok())
    {
        if (robot_state_available)
        {
            ...
            {
                p = planner.compute_multi_traj_cubic_prof_frames(0.0);
            }
            else if(t > init_time_slot && t <= traj_duration + init_time_slot)
            {

                p = planner.compute_multi_traj_cubic_prof_frames(t-init_time_slot);
                ...
                {
                    std::cout << "\n### Target " << current_target+1 << " raggiunto ###\n";
                    ...
                    {
                        current_target = 0;   // start again from the first target
                    }
